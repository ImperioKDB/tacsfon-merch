...
async function handler(req, res) {
  ...
  const { data: order, error: orderErr } = await supabaseAdmin
    .from('orders').select('id, status, payment_status, proof_url, user_id').eq('id', orderId).single()

  if (orderErr || !order) throw new ApiError('ORDER_NOT_FOUND', 'Order not found.', 404)
  if (!order.proof_url)   throw new ApiError('PROOF_NOT_FOUND', 'No proof uploaded for this order.', 400)

  if (payment_status === 'paid') assertValidTransition(order.status, 'confirmed')
