          ...
          <div className="mt-auto pb-10 space-y-6">
            {!loading && user ? (
              <div className="space-y-4">
                {isAdmin && (
                  <Link onClick={() => setIsOpen(false)} href="/admin" className="flex items-center gap-3 text-gold font-bold text-sm tracking-widest">
                    <LayoutDashboard size={18}/> ADMIN PANEL
                  </Link>
                )}
                <Link onClick={() => setIsOpen(false)} href="/profile" className="flex items-center gap-3 text-white font-bold text-sm tracking-widest">
                  <User size={18}/> MY ACCOUNT
                </Link>
                <button onClick={signOut} className="flex items-center gap-3 text-red-500 font-bold text-xs tracking-widest pt-4">
