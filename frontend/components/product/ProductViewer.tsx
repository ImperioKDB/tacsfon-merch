...
export default function ProductViewer({ imageUrl, modelUrl, productName, categoryName }: ProductViewerProps) {
  ...
  useEffect(() => { 
    if (threeError && hasImage) setActiveTab('image') 
  }, [threeError, hasImage])
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <button 
          ...
          disabled={threeError} 
          className={`px-4 py-2 text-xs font-bold border transition-all \${activeTab === '3d' ? 'bg-gold text-bg border-gold' : 'bg-surface text-text-secondary border-border'}`}
        >
          3D VIEW
        </button>
        {hasImage && (
          <button 
            ...
            className={`px-4 py-2 text-xs font-bold border transition-all \${activeTab === 'image' ? 'bg-gold text-bg border-gold' : 'bg-surface text-text-secondary border-border'}`}
          >
            IMAGE
          </button>
        )}
