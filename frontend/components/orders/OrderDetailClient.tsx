  ...
  return (
    <main className="min-h-screen px-4 py-10 md:px-8" style={{ background: 'var(--color-bg)' }}>
      <div className="max-w-2xl mx-auto space-y-5 animate-pulse">
        <div className="h-5 w-24 rounded-lg"  style={{ background: 'var(--color-surface-2)' }} />
...
export default function OrderDetailClient({ orderId }: Props) {
  ...
  // Open upload modal automatically if redirected from checkout with ?proof=uploaded
  // or if coming from skip ("proof not uploaded yet" state)
  useEffect(() => {
  ...
  // ── After Mark Received succeeds ─────────────────────────────────────────

  function handleReceivedSuccess() {
    setShowReceive(false)
    fetchOrder()   // re-fetch to get updated status
  }
        ...
        <div className="max-w-2xl mx-auto space-y-5">
          ...
          {showUploadBtn && (
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl
                         text-sm font-bold transition-all hover:opacity-90 active:scale-[.98]"
              style={{ background: 'var(--color-gold)', color: '#000' }}
            >
          ...
          {showReceiveBtn && (
            <button
              ...
              style={{ background: 'var(--color-gold)', color: '#000' }}
            >
              <span className="text-base">📦</span> Mark as Received
            </button>
          )}
          ...
          {showReceiptBtn && (
            <Link
              ...
              className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl
                         text-sm font-bold transition-all hover:opacity-90"
              style={{
