  ...
  const columns: Column<any>[] = [
    ...
    { key: 'act', label: 'Actions', render: r => (
      <div className="flex gap-4">
        <button onClick={() => { setEditingId(r.id); setForm({name:r.name, description:r.description||'', base_price:r.base_price, category_id:r.category_id, stock_type:r.stock_type}); setShowPanel(true); }} className="text-zinc-500 hover:text-gold"><Pencil size={18}/></button>
        <button onClick={() => handleDelete(r.id)} className="text-zinc-500 hover:text-red-500"><Trash2 size={18}/></button>
      </div>
    )}
  ]
          ...
          <div className="relative w-full max-w-md bg-black border-l border-zinc-800 p-8 h-full overflow-y-auto">
             ...
             <form onSubmit={handleSave} className="space-y-6">
                <div className="p-4 border-2 border-dashed border-zinc-800 text-center">
                    <input type="file" accept="image/png, image/jpeg, image/webp" onChange={e => setSelectedFile(e.target.files?.[0] || null)} className="w-full text-xs text-zinc-500" />
                    <p className="text-[9px] uppercase font-black text-amber-500 mt-2">NO HEIC / IPHONE FILES</p>
                </div>
                ...
                <button disabled={isSaving} className="w-full bg-gold text-black py-4 font-black uppercase tracking-widest">{isSaving ? 'Processing...' : 'Save Product'}</button>
             </form>
          </div>
        </div>
      )}
