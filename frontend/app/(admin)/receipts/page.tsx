...
export default function ReceiptsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      // apiFetch unwraps .data → { orders: [...] }
      const res = await apiFetch<any>('/admin/orders?status=received&limit=100')
  ...
  async function handleDownload(orderId: string) {
    setFetching(orderId)
    try {
      const res = await apiFetch<any>(`/admin/orders/${orderId}/receipt`)
      const url = res.signed_url ?? res.url ?? res.receipt_url
    ...
    <div style={{ padding: '24px 16px 80px', maxWidth: '900px' }}>
      ...
      {loading ? (
        ...
        <div style={{ padding: '56px 20px', textAlign: 'center', background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
          ...
          <p style={{ fontFamily: 'var(--font-display)', fontSize: '20px', letterSpacing: '0.08em', color: 'var(--text-muted)', textTransform: 'uppercase', margin: 0 }}>No completed orders yet</p>
        </div>
      ) : (
          ...
          {orders.map(o => (
            <div key={o.id} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '14px 16px', background: 'var(--bg-surface)', flexWrap: 'wrap' }}>
              <div style={{ flex: 1, minWidth: '160px' }}>
                <p style={{ margin: '0 0 2px', fontFamily: 'var(--font-mono)', fontSize: '12px', fontWeight: 700, color: A, letterSpacing: '0.06em' }}>
                  {formatOrderId(o.id)}
                </p>
                <p style={{ margin: 0, fontFamily: 'var(--font-body)', fontSize: '12px', color: 'var(--text-muted)' }}>
                  {o.user?.full_name ?? o.customer_name ?? '—'}
                </p>
              </div>
